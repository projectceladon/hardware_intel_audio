...
bool AudioStreamRoute::supportDeviceAddress(const std::string& streamDeviceAddress, audio_devices_t device) const
{
    Log::Verbose() << __FUNCTION__ << ": route device address " << mConfig.deviceAddress
                 ...
                 << ", verdict " << ((!device_distinguishes_on_address(device) && mConfig.deviceAddress.empty())
...
android::status_t AudioStreamRoute::dump(const int fd, int spaces) const
{
    ...
    snprintf(buffer, SIZE, "%*sStream Route: %s %s\n", spaces, "", getName().c_str(), (isOut() ? "playback" : "capture"));
    ...
    result.append(buffer);
    snprintf(buffer, SIZE, "%*s- requirePostDisable: %d\n", spaces + 4, "", mConfig.requirePostDisable);
    result.append(buffer);
    ...
    snprintf(buffer, SIZE, "%*s- supported use cases: %s\n", spaces + 4, "",
             isOut() ? "n/a" : InputSourceConverter::maskToString(mConfig.useCaseMask, ",").c_str());
    result.append(buffer);
    snprintf(buffer, SIZE, "%*s- channel control: %s\n", spaces + 4, "", mConfig.dynamicChannelMapsControl.c_str());
    result.append(buffer);
