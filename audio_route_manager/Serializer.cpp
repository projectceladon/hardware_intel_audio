                                       ...
                                       PtrSerializingCtx /*serializingContext*/)
{
    ...
    if (name.empty()) {
        Log::Error() << __FUNCTION__ << ": DevicePort: No attribute " << Attributes::name << " found.";
        return BAD_VALUE;
    }
    Log::Verbose() << __FUNCTION__ << ": DevicePort: attribute " << Attributes::name << "=" << name;
    string typeName = getXmlAttribute(root, Attributes::type);
    if (typeName.empty()) {
        Log::Error() << __FUNCTION__ << ": DevicePort: No attribute " << Attributes::type << " found.";
        return BAD_VALUE;
    }
    Log::Verbose() << __FUNCTION__ << ": DevicePort: attribute " << Attributes::type << "=" << typeName;
    string role = getXmlAttribute(root, Attributes::role);
    if (role.empty()) {
        Log::Error() << __FUNCTION__ << ": DevicePort: No attribute " << Attributes::role << " found.";
        return BAD_VALUE;
    }
    Log::Verbose() << __FUNCTION__ << ": DevicePort: attribute " << Attributes::role << "=" << role;
    audio_devices_t type = AUDIO_DEVICE_NONE;
    if (not DeviceConverter::toEnum(typeName, type)) {
        Log::Error() << __FUNCTION__ << ": DevicePort: Wrong " <<  typeName << "for attribute " << Attributes::type << " found.";
        return BAD_VALUE;
    }
    string address = getXmlAttribute(root, Attributes::address);
    if (not address.empty()) {
        Log::Verbose() << __FUNCTION__ << ": DevicePort: attribute " << Attributes::address << " = " << address;
    }
                                  ...
                                  PtrSerializingCtx /*ctx*/)
{
    ...
    Log::Verbose() << __FUNCTION__ << ": Route: attribute " << Attributes::sources << "=" << sourcesAttr;
                                    ...
                                    PtrSerializingCtx ctx)
{
    ...
    if (device.empty()) {
        ...
        if (not convertTo<string, uint32_t>(device, mixPortConfig.deviceId)) {
            Log::Error() << __FUNCTION__ << ": Invalid " << device << " for attribute " << Attributes::device;
            delete mixPort;
            return BAD_VALUE;
        }
        ...
        if (role == "sink") {
            ...
            if (route->mSink == name) {
                for (const auto &source : route->mSources) {
                    ...
                    if (port != nullptr) {
                        Log::Verbose() << __FUNCTION__ << ": adding " << port->mName << " to mix port" << name;
                        mixPortConfig.supportedDeviceMask |= port->mType;

                        if (not port->mAddress.empty()) {
                            mixPortConfig.deviceAddress = port->mAddress;
                            Log::Verbose() << __FUNCTION__ << ": adding @" << port->mAddress << " to mix port" << name;
                        }
                    }
                }
            }
                                   ...
                                   PtrSerializingCtx ctx)
{
    ...
    if (name != "primary") {
        Log::Warning() << __FUNCTION__ << ": Module " << name << " outside primary outside HAL Scope";
        return BAD_VALUE;
    }
