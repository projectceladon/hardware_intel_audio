...
{
    AUDIOUTILITIES_ASSERT(mPcmDevice == NULL, "alsa device already opened");
    AUDIOUTILITIES_ASSERT(deviceName != NULL, "Null card name");
                                  ...
                                  snd_pcm_access_t access, int soft_resample)
{
    ...
    err = snd_pcm_hw_params_set_format(mPcmDevice, params, AlsaAudioUtils::convertHalToAlsaFormat(config.getFormat()));
    if (err < 0) {
    ...
    if (err < 0) {
        Log::Error() << __FUNCTION__ << " Channels count " << config.getChannelCount() << " not available for " << s << " :" << snd_strerror(err);
        return err;
    }
    /* set the stream rate */
    err = snd_pcm_hw_params_set_rate_near(mPcmDevice, params, &rate, 0);
    if (err < 0) {
        Log::Error() << __FUNCTION__ << " Rate " << config.getRate() << "Hz not available for playback: " << snd_strerror(err);
        return err;
    }
    if (rate != config.getRate()) {
        Log::Error() << __FUNCTION__ << " Rate doesn't match (requested " << config.getRate() << "Hz, got " << rate << "Hz)";
        return -EINVAL;
    }
    /* set the buffer time */
